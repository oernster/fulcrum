...
`.coveragerc` gates the domain, the application and the infrastructure at 100%.
It omits the surfaces that are composition or framework glue: the UI, `main.py`,
the shared asset discovery, the application Protocol definitions, the version
module and `generate_icons.py`. Those carry no branching logic, so they are
verified by running the app rather than by the gate.
...
lines. The architectural rules are therefore tested, not merely documented.
...
and behaviour, then by eye on a real window. One caveat is worth knowing: a
