...
# bundle's non-binary files (read by the installer UI), payload/Fulcrum.zip
# holds the full app bundle for deployment and payload/LICENSE holds the
# licence text.
...
_PAYLOAD_ARCHIVE_NAME = "Fulcrum.zip"
# The application's asset resolver looks for these files beside the executable
# (the bundle root), so the icon paths below are bundle-root relative rather
# than nested inside an assets subdirectory.
_ICON_FILE_NAME = "fulcrum_256.png"
...
_SHORTCUT_ICON_FILE_NAME = "fulcrum.ico"
...
_UNINSTALLER_NAME = "FulcrumSetup.exe"
...
_UNINSTALL_KEY = r"Software\Microsoft\Windows\CurrentVersion\Uninstall\Fulcrum"
...
# removes that registration. Must match the app's APP_APPUSERMODELID.
_APP_AUMID = "uk.codecrafter.fulcrum"
...
_INSTALLER_LOG_NAME = "fulcrum-installer.log"
...
_TEXT_MUTED = "#9aa3af"
_ACCENT = "#f59e0b"
# Buttons never wear a filled accent: the green hover ring is hard to read
# against an amber or light blue fill, so every button is dark grey with a
# light blue caption and only the ring changes on hover. The amber remains
# on the headings only.
_BUTTON_BLUE = "#7fb0ff"
...
def _is_app_running() -> bool:
    """Return True when fulcrum.exe appears in the task list (best effort)."""
