...
Dropping onto a unit moves the dragged item inside it; dropping in the upper
or lower band of a row inserts beside that row; holding Ctrl copies instead of
moving. Legality is delegated to the draft (via can_place), so an illegal drop
(into a team, into the item's own subtree or a tier under a lower tier) shows
the forbidden cursor exactly as a file manager would. The tree never moves
rows itself; the drop is handed to the pane, which updates the draft and
rebuilds, so the view always reflects the model.
"""
...
# The top and bottom band of a row (as a fraction of its height) that means
# "insert beside" rather than "drop inside".
_BESIDE_BAND = 4
...
class DraftTree(QTreeWidget):
    ...
    def __init__(self, can_place, on_drop, parent=None) -> None:
    ...
    def _placement(self, pos):
        ...
        return (item.data(0, _ROLE_ID), None)
