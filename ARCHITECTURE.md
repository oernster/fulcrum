...
- **UI** (`fulcrum/ui`): PySide6 widgets and dialogs (the board, the navigable org map, the organisation editor, the guide, the glossary, the book background and the about/licence dialogs), a client of the application only. A `ui_scale` factor set once at startup keeps the whole interface sized to the screen.
