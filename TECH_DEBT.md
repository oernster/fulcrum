...
- **The coverage omissions of the installer's Qt and side-effect modules** (`app.py`, `installer_bundle.py`, `installer_lifecycle.py`, `installer_ops.py`, `installer_theme.py`, `installer_widgets.py`, `installer_window.py`). The decisions were lifted into `installer_logic.py`, which is gated at 100%; what remains is widgets, a registry and subprocess edge plus the composition that wires them. Testing `installer_ops.py` would mean writing to the real HKCU hive.
